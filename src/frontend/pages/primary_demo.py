...
def _citations_to_pills(answer: str, evidence: list = None) -> str:
    ...
    def _pill(m):
        ...
        if evidence and 0 < n <= len(evidence):
            ...
            if len(excerpt) > 200: excerpt = excerpt[:200] + "..."
...
def _render_confidence_bar(evidence: list):
    ...
    if counts["blue"]:
        ...
        c = counts["blue"]
        html += f"<div class='conf-segment' style='width:{w}%; background:#3b82f6;' title='FDA Labels: {c}'></div>"
    if counts["red"]:
        w = (counts["red"] / total) * 100
        c = counts["red"]
        html += f"<div class='conf-segment' style='width:{w}%; background:#ef4444;' title='FAERS Reports: {c}'></div>"
    if counts["purple"]:
        w = (counts["purple"] / total) * 100
        c = counts["purple"]
        html += f"<div class='conf-segment' style='width:{w}%; background:#7c3aed;' title='Knowledge Graph: {c}'></div>"
    html += "</div>"
    return html
    ...
    if not r:
        ...
        with meta_cols[1]:
            ...
            st.markdown(f"<div style='font-size:13px;'>{llm_icon} <b>Generator:</b> {llm_label}</div>",
                        unsafe_allow_html=True)
        with meta_cols[2]:
            embed_label = "Vertex AI Embeddings" if r.get("method") else "TF-IDF"
...
def render_metrics():
    ...
    if r:
        ...
        st.markdown(f"- **Embedding model:** Vertex AI text-embedding-004")
